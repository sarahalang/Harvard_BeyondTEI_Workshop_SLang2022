...
\mycommand{<?xsl-stylesheet type="text/xsl" href="mein.xsl"?>}{processing instructions  (optional)}
\bigskip
...
\bg{w3schools}{white}{entities}~ `protected' characters because they have a meta meaning in XML like: \\
\mycommand{&lt;}{<}
\mycommand{&gt;}{>}
\mycommand{&amp;}{\&}
...
\bg{w3schools}{white}{XQuery}~query langauge for XML databases \\
...
\begin{itemize}
    \item \textbf{@n} (label) \item \textbf{@type} (typing) \item \textbf{xml:id} (unique identifier) \item \textbf{xml:lang} (language) \item \textbf{@rend} (rendering) \item \text{@ana} (interpretation)
\end{itemize}
\end{multicols}
...
\bg{w3schools}{white}{<präfix:name>}~
...
\footnotesize
\bg{w3schools}{white}{\emph{Named Entities} \& Indirekte reference}\\
...
%-----------------------------------------------------
\begin{frame}{Making the TEI your own}
\small
\metroset{block=fill}
