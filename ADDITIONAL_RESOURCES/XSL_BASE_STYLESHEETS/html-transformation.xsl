...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="/">
        ...
        <html>
            ...
            <body>
                <h1>List of personal names</h1>
                

                    <h3>How often does each name appear in the text?</h3>
                    <ul>
                ...
                <xsl:for-each select="//t:div[$thispart]/t:div[@type= 'chapter']">
                    ...
                    <h3><xsl:value-of select="t:head"/><xsl:text>:: </xsl:text><xsl:value-of select="$chapter"/></h3>
                    <ul>
                        <xsl:for-each select="//t:person">
                            ...
                            <li>
                                <xsl:value-of select="."/>
                                <xsl:text>-------</xsl:text>
                                <xsl:value-of select="count(//t:div[$thispart]/t:div[$chapter]//t:persName[@ref = $self])"/>
                            </li>
                        </xsl:for-each>
                    </ul>
                </xsl:for-each>
